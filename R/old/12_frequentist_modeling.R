...
conv_vars_to_pic <- function(df, tip_label = NULL, phylo_tree = NULL, vars_to_conv = NULL){
    if (is.null(vars_to_conv)) stop("Which variables should be converted?")
    if (is.null(tip_label)) stop("Which variable contains the tip_labels?")
    
    conv_var <- function(col_ind, df, phylo_tree){
        trait_vec <- as.vector(df[[col_ind]])
        names(trait_vec) <- df[[tip_label]]
        trait_contrast <- ape::pic(trait_vec, phylo_tree)
    }
    # full df
    df_contrasts <- data.frame(do.call(cbind, lapply(vars_to_conv, conv_var, df, phylo_tree)))
    names(df_contrasts) <- names(df)[vars_to_conv]
    df_contrasts
}


stats_mod <- all_stats %>% 
    dplyr::select(tip_label,TPM70_ratio, num_alleles_mean, SSD, #BreedingType, 
        male_weight, logAbundance, Generation_time, mratio_mean,
        obs_het_mean, mean_allele_range,  prop_low_afs_mean, #IUCN_rating,
        nloc, nind) %>% 
        data.frame()

stats_mod_contr <- conv_vars_to_pic(stats_mod, tip_label = "tip_label", phylo_tree = tree_final, 
                                    vars_to_conv = 2:ncol(stats_mod))

#ggduo(stats_mod_contr)

mod0 <- lm(TPM70_ratio ~ num_alleles_mean + mratio_mean + obs_het_mean + mean_allele_range + prop_low_afs_mean, stats_mod_contr)
summary(mod0)
out <- regr(mod0)
out

mod1 <- lm(num_alleles_mean ~ SSD + logAbundance + Generation_time, stats_mod_contr)
summary(mod1)
calc.yhat(mod1)

mod2 <- lm(TPM70_ratio ~ SSD + logAbundance + Generation_time, stats_mod_contr)
summary(mod2)
calc.yhat(mod2)



seal <- comparative.data(tree_final, data.frame(stats_mod ), "tip_label", vcv = TRUE)
mod <- pgls(num_alleles_mean ~ TPM70_ratio, seal) #  + SSD + Generation_time + BreedingType
sum_mod <- summary(mod)
sum_mod$r.squared
...
trait_vec <- as.vector(stats_mod$num_alleles_mean)
names(trait_vec) <- stats_mod$tip_label
trait_contr2 <- ape::pic(trait_vec, tree_final)
trait_contr

plot(trait_contr1, trait_contr2)
plot(trait_vec, trait_contr)
