...
# load model output
mod_beta <- read_delim("data/processed/models/mod_het_abc_beta.txt", delim = " ")
mod_R2 <- read_delim("data/processed/models/mod_het_abc_R2.txt", delim = " ")
mod_SC <- read_delim("data/processed/models/mod_het_abc_SC.txt", delim = " ")


p1 <- ggplot(aes(bot, TPM80_ratio), data = stats_mod) +
    geom_line(stat = "smooth", method = "lm", col = "lightgrey") + #, aes(fill = BreedingType)
    geom_ribbon(stat='smooth', method = "lm", alpha = 0.1, colour=NA) +
    geom_point(size = 4,  alpha = 0.5) + # abc_out
    geom_point(size = 4, shape = 21, col = "black") + #, aes(fill = BreedingType)
    #scale_color_viridis(option = "magma", direction = -1,
    #    name = "ABC bottleneck \nprobability %", labels=c("0", "50", "100"), breaks = c(0.05,0.5,1)) +
    theme_martin() +
    xlab("ABC bottleneck model probability") +
...
p_final
ggplot2::ggsave('figures/hetexc_vs_abc.jpg',p_final,  width=7, height=4)
