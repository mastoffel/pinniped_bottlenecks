...
# seals_sub <- seals[c(4, 5,6, 12, 13, 16:50)]
# # seals_sub <- seals[c(5,6, 12, 13, 31:49)]
# seals_sub %>% gather(-mean_SSD, -TPM70_ratio, -family, key = "var", value = "value") %>%
#     ggplot(aes(x = mean_SSD, y = value, colour=factor(family))) +
#     geom_point() +
...
# names(seals)
# seals_sub <- seals[c(4, 5,6, 12, 13, 16:52)]
# # seals_sub <- seals[c(5,6, 12, 13, 31:49)]
# seals_sub %>% gather(-nloc, -nind, key = "var", value = "value") %>%
#     ggplot(aes(x =value , y = nloc)) +
#     geom_point() +
#     geom_smooth(method = "lm") +
#     facet_wrap(~ var, scales = "free") 
...
p_bot_ratio <- ggplot(bot, aes(x= variable, y = species, fill = value)) + 
    ...
    theme(plot.title=element_text(hjust=0),
        ...
        legend.position="non",
        plot.margin=unit(c(10,1,10,1),"points")) +
...
p_diversity <- ggplot(bot, aes(x= variable, y = species, fill = value)) + 
    ...
    scale_fill_gradientn(colours=c("#081d58", "#253494", "#1d91c0", "#7fcdbb","#c7e9b4", "#ffffd9"),
        name = "standardised \nvalue")  +
    scale_x_discrete(labels=c( "ALLRI", "NEIDI")) +
    # scale_y_discrete(labels = rev(seal_names_real)) +
...
grid.arrange(p_bot_ratio, p_diversity, g2_plot, widths=c(0.4, 0.15, 0.45))

cclist <- list(p_bot_ratio, p_diversity, g2_plot)
cclist$ncol <- 3
do.call(grid.arrange, cclist)
