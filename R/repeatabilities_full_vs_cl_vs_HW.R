# Calculating repeatabilities of bottleneck signatures and summary statistics across 
# multiple datasets to confirm robustness of results.
...
# check whether full and non-HW data are similar using repeatabilities
...
seals <- read_csv("data/processed/all_stats_tree_30.csv")
seals_cl <- read_csv("data/processed/all_stats_tree_30_HW.csv")
all_seals <- rbind(seals, seals_cl)
...
calc_rpts <- function(var_in_all_stats){
    ...
    out <- data.frame(variable = var_in_all_stats, R = rpt_temp$R, CI = rpt_temp$CI_emp, 
        row.names = NULL) %>% 
        mutate_if(is.numeric, funs(round(., 3))) %>% 
        dplyr::rename(R = species, CI25 = CI.2.5., CI975 = CI.97.5.) %>% 
        mutate(CI = paste0("[",CI25," ", CI975, "]")) %>% 
        dplyr::select(variable, R, CI)
    out
}
...
kable(all_rpts_df, format = "latex",  escape = F, 
    ...
    kable_styling(latex_options = c( "scale_down")) %>% 
    row_spec(0, bold = TRUE) %>% 
    kable_as_image("SupTab6", keep_pdf = TRUE)
