...
plotting_sequence <- c("weddell_seal", "leopard_seal" , "ross_seal", "crabeater_seal", "ses", "nes",
    "hawaiian_monk_seal", "mediterranean_monk_seal", "lagoda_ringed_seal", 
    "baltic_ringed_seal", "saimaa_ringed_seal", "arctic_ringed_seal", "harbour_seal_waddensee",
...
# diversity
stand_div_lf <- stand_div %>% melt(id.vars = c("species","common", "latin"))
...
p_div <- ggplot(stand_div_lf, aes(x = value, y = species, shape = variable, col = species, fill = species)) + 
    geom_point(size = 3, alpha = 0.4) +
    # geom_jitter(height = 0.1, width = 0, size = 2, alpha = 0.6) +
    scale_shape_manual(values = c(21,24,22,23)) +
    scale_color_manual(values = rep(c("black", "black"), times = 14), guide = FALSE) +
    scale_fill_manual(values = rep(c("black", "grey"), times = 14), guide = FALSE) +
